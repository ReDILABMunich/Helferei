...
class FormKnowledge:
    ...
    def _score(self, q: str, names: List[str]) -> float:
        ...
        for n in (names or []):
            ...
            q_words = set(q.split())
            if n_words and n_words.issubset(q_words):
                best = max(best, 40)
                continue
            ...
            if len(q_words) >= 1 and len(n_words) >= 1:
                word_matches = sum(
                    1 for qw in q_words
                    if any(SequenceMatcher(None, qw, nw).ratio() >= 0.8 for nw in n_words)
                )
                if word_matches == len(q_words):
                    coverage = word_matches / max(len(q_words), len(n_words))
                    best = max(best, int(coverage * 60))
            # Fuzzy match — at least one word pair must match (min 4 chars)
            if len(q_words) >= 1 and len(n_words) >= 1:
                best_word_match = max(
                    (SequenceMatcher(None, qw, nw).ratio()
                     for qw in q_words for nw in n_words
                     if len(qw) >= 4 and len(nw) >= 4),
                    default=0
                )
                if best_word_match >= 0.9:
