...
# GLOSSARY
#
# Reference dictionary of German tax terms (76 entries).
#
# STATUS: Future-ready. Currently only the entries `umsatz_gewinn` and
# `steuernummer` are reachable from the code path — they are injected into
# the response when the intent parser sets `detected_confusion` to one of
# `umsatz_gewinn`, `steuernummer_ust`, or `estimate_vs_fact`
# (see agent.py — ContextBuilder.from_glossary).
#
# The remaining 74 entries are intentionally kept as a base for extending
# the confusion-detection logic to more concept pairs.
# ─────────────────────────────────────────────────────────────────────────────
